'''MSNN on the Northwestern-UCLA Multiview 3D dataset.

NOTE: unlike the NTU scripts, this one consumes SSIs that were *precomputed*
by `preprocessing/UCLA/compute_ssm_ucla_cv.py` and streams them through the
`generate_train` / `generate_test` generators.  It does not contain the
learnable `MetricLayer`, i.e. it predates the integration of the SSI
computation into the network described in Sec. I of the paper.
'''
import time
import numpy as np
...
def get_model(layers_num,joint_num, max_len, stride_set,class_num, R3,spp_numbers,scale):
    ...
    spp_layer = wrappers.TimeDistributed(SpatialPyramidPooling(spp_numbers),name = 'TSPP'+'-'+scale) 
    gru_input = spp_layer(c3d_out3)
    ...
    model = Model(inputs=[main_input], outputs=[z_out_f])
    return model
...
def train_lstms(layers_num,batch_size,joint_num,Regs,filepath,stride_set,spp_numbers,epoch,fine_tune = False,scale='1'):
    ...
    if fine_tune == True:
        model.load_weights('model_scale'+scale+'_trained.h5')   
    fc2 = Dense(class_num,activation='softmax',kernel_regularizer = R3,bias_regularizer = R3,name = 'FC2'+'-'+scale)
    main_output = fc2(model.outputs[0])
    ...
    # sgd = op.SGD(lr=0.01, momentum=0.9, decay=1e-6)
...
if __name__=='__main__':
    ...
    ###########################################################
    joint_num1  = 7
    spp_numbers1 = [1] # sum([ x**2 for x in spp_numbers1 ])*16 for the spp layer
    ...
    model_scale1,test_score[0]=train_lstms(layers_num1,batch_size,joint_num1,Regs,filepath,stride_set,spp_numbers1,epoch=40,fine_tune = False,scale='1')
    model_scale1.save('uclamodel_scale1_v2_ta.h5')    
    #########################################################
    joint_num2  = 12
    spp_numbers2 = [1,2]
    layers_num2 = [joint_num2,8,16,32,100,100,sum([ x**2 for x in spp_numbers1 ])*32] ##128, 32,32,100,100##64,64,64,200 [input_layer,lstm1-lstm3,fc1_layer,fc2_layer] [joint_num2,80,80,80,160]
    model_scale2,test_score[1]=train_lstms(layers_num2,batch_size,joint_num2,Regs,filepath,stride_set,spp_numbers2,epoch=40,fine_tune = True,scale='2')
    model_scale2.save('uclamodel_scale2_v2_ta.h5')
    ##########################################################
    joint_num3  = 20
    spp_numbers3 = [1,2,3]
    layers_num3 = [joint_num3,16,32,64,100,100,sum([ x**2 for x in spp_numbers1 ])*64] ##576, 32,64,64,200 [input_layer,lstm1-lstm3,fc1_layer,fc2_layer] [joint_num2,100,100,100,200]
    model_scale3,test_score[2]=train_lstms(layers_num3,batch_size,joint_num3,Regs,filepath,stride_set,spp_numbers3,epoch=40,fine_tune = True,scale='3')
    model_scale3.save('uclamodel_scale3_v2_ta.h5')
    ###########################################################
    model_scale1 = get_model(layers_num1,joint_num1, max_len, stride_set,class_num, R3,spp_numbers1,scale = '1')
    fc2 = Dense(class_num,activation='softmax',kernel_regularizer = R3,bias_regularizer = R3,name = 'FC2'+'-'+'1')
    main_output = fc2(model_scale1.outputs[0])
    model_scale1 = Model(inputs = model_scale1.inputs,outputs = main_output)
    model_scale2 = get_model(layers_num2,joint_num2, max_len, stride_set,class_num, R3,spp_numbers2,scale = '2')
    fc2 = Dense(class_num,activation='softmax',kernel_regularizer = R3,bias_regularizer = R3,name = 'FC2'+'-'+'2')
    main_output = fc2(model_scale2.outputs[0])
    model_scale2 = Model(inputs = model_scale2.inputs,outputs = main_output)
    model_scale3 = get_model(layers_num3,joint_num3, max_len, stride_set,class_num, R3,spp_numbers3,scale = '3')
    fc2 = Dense(class_num,activation='softmax',kernel_regularizer = R3,bias_regularizer = R3,name = 'FC2'+'-'+'3')
    main_output = fc2(model_scale3.outputs[0])
    model_scale3 = Model(inputs = model_scale3.inputs,outputs = main_output)
