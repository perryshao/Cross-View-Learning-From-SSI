...
def get_model(
    layers_num, joint_num, max_len, stride_set, class_num, R3, spp_numbers, spp_numbers_a, scale
):
    """Build one SSI stream up to its attention-weighted CNN features."""
    # layers_num: joints, conv1, conv2, conv3, attention width, feature width.
    main_layers = layers_num
    ...
    ssm_input = MetricLayer(joint_num, kernel_regularizer=R3, name='metric_layer' + '-' + scale)(
        ssm_input
    )
    ...
    fc2_ta = Dense(
        CLIP_LENGTH, activation='relu', kernel_regularizer=R3, bias_regularizer=R3, name='FC2_ta'
    )
...
if __name__ == '__main__':
    ...
    spp_numbers1_a = [7]
    layers_num1 = [joint_num1, 4, 8, 16, 100, sum([x**2 for x in spp_numbers1]) * 16]
    joint_num2 = 12 * 2
    spp_numbers2 = [4]
    spp_numbers2_a = [12]
    layers_num2 = [joint_num2, 8, 16, 32, 100, sum([x**2 for x in spp_numbers2]) * 32]
    joint_num3 = 25 * 2
    spp_numbers3 = [4]
    spp_numbers3_a = [20]
    layers_num3 = [joint_num3, 16, 32, 64, 100, sum([x**2 for x in spp_numbers3]) * 64]
    ...
    fc_softmax = Dense(
        class_num, activation='softmax', kernel_regularizer=R3, bias_regularizer=R3, name='FC2'
    )
    ...
    plot_confusion_matrix(
        matrix, classes=class_names, normalize=True, title='Normalized confusion matrix'
    )
