...
MAT_NAMES = dict(
    train_mat='data_cv3_scale',
    train_key='data_cv3',
    test_mat='data_cv1_scale',
    test_key='data_cv1',
)
...
def get_model(
    layers_num, joint_num, max_len, stride_set, class_num, reg, spp_numbers, spp_numbers_a, scale
):
    """Build one stream of the MSNN for SSIs of a single scale."""
    main_layers = layers_num[0:5]
    conv_feats_dim = layers_num[6]

    main_input = Input(
        shape=(max_len, main_layers[0], 3), dtype='float32', name='main_input-' + scale
    )

    # Pairwise joint graph, then the SSI itself (Eq. 1-3).
    ...
    ssm_input = MetricLayer(joint_num, kernel_regularizer=reg, name='metric_layer-' + scale)(
        ssm_input
    )
    ssm_input = Lambda(flattenSSM, flattenSSM_output_shape)(ssm_input)

    # Spatiotemporal convolutional module (Sec. III-B1).
    ...
    # Temporal attention branch, reading the SSIs directly.
    spp_layer1 = wrappers.TimeDistributed(
        SpatialPyramidPooling(spp_numbers_a), name='TSPP1-' + scale
    )
    ...
    t_atten = Dense(
        CLIP_LENGTH, activation='relu', kernel_regularizer=reg, bias_regularizer=reg, name='FC2_ta'
    )(t_atten)
    t_atten = Permute((2, 1))(t_atten)
    t_atten = Dense(
        conv_feats_dim,
    ...
    # LSTM sequence-encoding module (Sec. III-B2), gated by the attention.
    spp_layer2 = wrappers.TimeDistributed(
        SpatialPyramidPooling(spp_numbers), name='TSPP2-' + scale
    )
    ...
    z_out_f = Lambda(
        lambda x: K.sum(x, axis=1), output_shape=lambda s: (s[0], s[2]), name='Lambda2-' + scale
    )(lstm_output2)
...
def train_stream(
    ...
    """Pre-train a single stream on SSIs of one scale."""
    global_start_time = time.time()

    print('> Loading data... ')
    x_train, y_train, x_test, y_test = load_raw(filepath, scale)
    print('x_train shape:', x_train.shape)  # (40091, 150, rows, cols, channels)
    print('y_train shape:', y_train.shape)  # (40091, 60)
    print('x_test shape:', x_test.shape)  # (16487, 150, rows, cols, channels)
    print('y_test shape:', y_test.shape)  # (16487, 60)
...
if __name__ == '__main__':
    ...
    # Pre-train the three streams. layers_num is
    # [joints, conv1, conv2, conv3, lstm, fc1, conv_feats_dim]; the last entry
    # is the flattened TSPP output, sum(n^2 for n in spp_numbers) * conv3.
    joint_num1 = 7 * 2
    spp_numbers1 = [4]
    spp_numbers1_a = [7]
    layers_num1 = [joint_num1, 4, 8, 16, 100, 100, sum(x**2 for x in spp_numbers1) * 16]
    model_scale1, test_score[0] = train_stream(
        layers_num1,
        batch_size,
        joint_num1,
        reg,
    ...
    spp_numbers2 = [4]
    spp_numbers2_a = [12]
    layers_num2 = [joint_num2, 8, 16, 32, 100, 100, sum(x**2 for x in spp_numbers2) * 32]
    model_scale2, test_score[1] = train_stream(
        layers_num2,
        batch_size,
        joint_num2,
        reg,
    ...
    spp_numbers3 = [4]
    spp_numbers3_a = [20]
    layers_num3 = [joint_num3, 16, 32, 64, 100, 100, sum(x**2 for x in spp_numbers3) * 64]
    model_scale3, test_score[2] = train_stream(
        layers_num3,
        batch_size,
        joint_num3,
        reg,
    ...
    # Rebuild the streams and reload their weights for the fusion stage.
    model_scale1 = get_model(
        layers_num1,
        joint_num1,
        max_len,
        stride_set,
    ...
    model_scale3.name = 'model_3'  # avoid name clashes between the streams
    ...
    if FUSION_MODE == 'feature':
        ...
        sub_model1 = Model(
            inputs=model_scale1.inputs, outputs=model_scale1.layers[-2].output, name='stream_1'
        )
        sub_model2 = Model(
            inputs=model_scale2.inputs, outputs=model_scale2.layers[-2].output, name='stream_2'
        )
        sub_model3 = Model(
            inputs=model_scale3.inputs, outputs=model_scale3.layers[-2].output, name='stream_3'
        )
    ...
    checkpoint = ModelCheckpoint(
        './logs/' + model_filepath, monitor='val_acc', save_weights_only=True
    )
    history = mgpu_model.fit(
