...
datapath = '/home/data/HRI-UESTC/AV/' ##protocol (1,3,5,7) vs (2)
...
for seq in seqs:
    ...
    if seq_name_list[1][1:] not in ['8']: ##protocol: arbitrary-view 
    ...
    elif seq_name_list[1][1:] in ['8']:  ##protocol: cross-subject
        print('processing the %sth sequence' % seq)
        seq_mat = mat_path+seq
        try:
            file = io.loadmat(seq_mat)
        ...
        if ske_xyz != []:
            ...
            # test_data_cv = test_data_cv+segments
            # test_labels = test_labels + [int(seq_name_list[0][1:])]*10
            # frame_length = frame_length + [int(ske_xyz.shape[0])]*10
