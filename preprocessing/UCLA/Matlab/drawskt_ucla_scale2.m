function drawskt_ucla_scale2()
    %% global parameters
    % sf: start frame
    % ef: ending frame
    J = [1     2     3     3    5    3    7    1    11   1   9;
       2     3     4     5    6    7    8    11   12   9   10];
    train_samples = 937;
    test_samples = 463;
    joint_number = 12;
    actions = 10;
...
function draw_samples(samples, frames, filename, filepath, labels_dataset, dataset, actions, ...
    ...
    data = h5read([filepath filename], ['/' dataset], [1, 1, samples], [joint_number * 3, ...
        frames, 1]);
    skeleton = reshape(data', [frames, joint_number, 3]);

    % apply affine transformation on skeleton
    % skeleton = affine_tranform(skeleton,joint_number);
    ...
    for s = 1:size(X, 2)
        ...
        axis off;
        ssi = compute_ssm(skeleton(s, :), joint_number);
        ...
        axis off;
        saveas(gcf, ['output2_', num2str(s)], 'bmp')
        pause(1 / 15)
    end
end

function ssi = compute_ssm(skeleton, joint_number)
    skeleton = reshape(skeleton, [3, joint_number])';
    ssi = pdist2(skeleton, skeleton);
    % ssi = ssi/(max(max(ssi))+1e-8);
end

function skeleton = affine_tranform(skeleton, joint_number)
