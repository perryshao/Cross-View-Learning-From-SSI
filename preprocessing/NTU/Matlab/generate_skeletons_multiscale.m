function generate_skeletons_multiscale(Data_Path)
    ...
    bodyJoints{2} = [HEAD_1 C7_1 T10_1 STRN_1 RELB_1 RFIN_1 LELB_1 LFIN_1 RKNE_1 RTOE_1 LKNE_1 ...
        LTOE_1];
    bodyJoints{3} = [HEAD_1 HEAD_A C7_1 T10_1 STRN_1 RSHO_1 RELB_1 RWRA_1 RFIN_A RFIN_B RFIN_1 ...
        LSHO_1 LELB_1 LWRA_1 LFIN_A LFIN_B LFIN_1 RFWT_1 RKNE_1 RANK_1  RTOE_1 LFWT_1 LKNE_1 ...
        LANK_1  LTOE_1];
    body_num = 2;
    FullJointNum = 25;
    ...
    for n = 1:test_n_cv
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    test_data_cv_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 1);
                    test_data_cv_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 2);
                    test_data_cv_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:train_n_cv
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    train_data_cv_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 1);
                    train_data_cv_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 2);
                    train_data_cv_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 3);
                end
            end
        end
    ...
    for n = 1:test_n_cs
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    test_data_cs_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 1);
                    test_data_cs_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 2);
                    test_data_cs_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:train_n_cs
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    train_data_cs_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 1);
                    train_data_cs_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 2);
                    train_data_cs_scale1{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:test_n_cv
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    test_data_cv_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 1);
                    test_data_cv_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 2);
                    test_data_cv_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:train_n_cv
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    train_data_cv_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 1);
                    train_data_cv_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 2);
                    train_data_cv_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:test_n_cs
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    test_data_cs_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 1);
                    test_data_cs_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 2);
                    test_data_cs_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:train_n_cs
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    train_data_cs_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 1);
                    train_data_cs_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 2);
                    train_data_cs_scale2{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:test_n_cv
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    test_data_cv_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 1);
                    test_data_cv_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 2);
                    test_data_cv_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        test_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:train_n_cv
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    train_data_cv_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 1);
                    train_data_cv_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 2);
                    train_data_cv_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        train_data_cv{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:test_n_cs
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    test_data_cs_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 1);
                    test_data_cs_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 2);
                    test_data_cs_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        test_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + (bodyJoints{scale}(j) ...
                        - 1) * 3 + 3);
                end
            end
    ...
    for n = 1:train_n_cs
        ...
        for i = 1:framecount
            for j = 1:JOINT_NUM
                for k = 1:body_num
                    train_data_cs_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 1);
                    train_data_cs_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 2);
                    train_data_cs_scale3{n, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                        train_data_cs{n, 1}(i, (k - 1) * FullJointNum * 3 + ...
                        (bodyJoints{scale}(j) - 1) * 3 + 3);
                end
            end
