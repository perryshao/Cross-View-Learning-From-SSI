...
class SpatialPyramidPooling(Layer):
    ...
    def call(self, x, mask=None):
        ...
        # pool_max = max(self.pool_list)
        # pad_num = pool_max - num_rows
        # if self.dim_ordering == 'th':
        #     x = K.switch(K.greater(pool_max,num_rows), K.spatial_2d_padding(x,padding=((0,pad_num),(0,pad_num)),data_format='channels_first'), x)
        # elif self.dim_ordering == 'tf':
        #     x = K.switch(K.greater(K.cast(pool_max,'int32'),num_rows), K.spatial_2d_padding(x,padding=((0,pad_num),(0,pad_num)),data_format='channels_last'), x)
        # input_shape = K.shape(x)    
        # if self.dim_ordering == 'th':
        #     num_rows = input_shape[2]
        #     num_cols = input_shape[3]
...
class SpatialPyramidPooling3D(Layer):
    ...
    def call(self, x, mask=None):
        ...
        if self.dim_ordering == 'th':
            num_depths = input_shape[2]
            num_rows = input_shape[3]
            num_cols = input_shape[4]
        elif self.dim_ordering == 'tf':
            num_depths = input_shape[1]
            num_rows = input_shape[2]
            num_cols = input_shape[3]
        depth_length = [K.cast(num_depths, 'float32') / i for i in self.pool_list]
        row_length = [K.cast(num_rows, 'float32') / i for i in self.pool_list]
        col_length = [K.cast(num_cols, 'float32') / i for i in self.pool_list]
                ...
                for jy in range(num_pool_regions):
                    for ix in range(num_pool_regions):
                        for iz in range(num_pool_regions):
                            z1 = iz * depth_length[pool_num]
                            z2 = iz * depth_length[pool_num] + depth_length[pool_num]
                            x1 = ix * col_length[pool_num]
                            x2 = ix * col_length[pool_num] + col_length[pool_num]
                            y1 = jy * row_length[pool_num]
                            y2 = jy * row_length[pool_num] + row_length[pool_num]
