function drawskt_uestc_skeleton()
    %% global parameters
    % sf: start frame
    % ef: ending frame
    ...
    joint_number = 25;
    actions = 40;
    filepath = 'C:\Users\perry\Desktop\HRI-UESTC\';
    %%%%%%%%%%%%%%%%%%%%% --view_1--%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% draw the view_1
    frames = 150; % full length = 32
    samples = 24; % 238
    ...
    h5disp([filepath 'frame_length.h5'], ['/' 'frame_length'])
    frame_length =  h5read([filepath 'frame_length.h5'], ['/' 'frame_length'], [1], [8000]);
    save frame_length frame_length
    figure(1)
    draw_samples(samples, frames, filename, filepath, labels_dataset, dataset, actions, ...
        joint_number, J);
    %%%%%%%%%%%%%%%%%%%%% --view_2--%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% draw the view_2
    frames = 30; % full length = 26
    samples = 161;
    filename = 'Train_Raw_cv3.h5';
    labels_dataset = 'y_train';
    dataset = 'x_train';
...
function draw_samples(samples, frames, filename, filepath, labels_dataset, dataset, actions, ...
    joint_number, J)
    ...
    data = h5read([filepath filename], ['/' dataset], [1, 1, 1, samples], [3, joint_number, ...
        frames, 1]);
    skeleton_p = permute(data, [3, 2, 1]);
    skeleton = reshape(skeleton_p, [frames, 3 * joint_number]);
    squeeze(skeleton_p(1, :, :));
    ...
    for s = 1:size(X, 2)
        ...
        % ssi1 = h5read([filepath 'Trainset3.h5'],['/'
        % 'X_train'],[1,1,1,s,samples],[1,joint_number,joint_number,1,1]);
        %     ssi1 = squeeze(ssi1);
...
function ssi = compute_ssm(skeleton)
    skeleton = reshape(skeleton, [3, 25])';
    ssi = pdist2(skeleton, skeleton);
    % ssi = (ssi-mean(mean(ssi)))/(std(std(ssi))+1e-8);
    ssi = (ssi - min(min(ssi))) / (max(max(ssi)) - min(min(ssi)) + 1e-8);
