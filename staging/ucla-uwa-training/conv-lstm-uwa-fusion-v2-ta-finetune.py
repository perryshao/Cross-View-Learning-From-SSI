'''MSNN fine-tuning on the UWA3D-II Multiview Activity dataset.

    python conv-lstm-uwa-fusion-v2-ta-finetune.py -i 100 -lr 0.0001 -b 8 -d 1e-6

NOTE: consumes SSIs precomputed by `preprocessing/UWA/compute_ssm_uwa_cv.py`;
...
def get_model(layers_num,joint_num, max_len, stride_set,class_num, R3,spp_numbers,spp_numbers_a,scale,summary=False):
    ...
    fc1_ta = Dense(main_layers[4]/2, activation='relu',kernel_regularizer = R3,bias_regularizer = R3,name = 'FC1_ta_uwa')
    t_atten = fc1_ta(t_atten)
    t_atten = Permute((2, 1))(t_atten)
    fc2_ta = Dense(clip_length, activation='relu',kernel_regularizer = R3,bias_regularizer = R3,name = 'FC2_ta_uwa')
    t_atten = fc2_ta(t_atten)
    t_atten = Permute((2, 1))(t_atten)
    fc3_ta = Dense(layers_num[-1], activation='sigmoid',kernel_regularizer = R3,bias_regularizer = R3,name = 'FC3_ta_uwa')
...
def train_lstms(layers_num,batch_size,joint_num,Regs,filepath,stride_set,spp_numbers,spp_numbers_a,epoch,freeze = False,transfer =True,scale='1'):
    ...
    if transfer == True:
        ...
        model.load_weights('model_scale'+scale+'_ptrained_ucla.h5',by_name=True)
    ...
    if freeze == True:
        ...
        # for l in model.layers[:-6]: # freeze the convlution layers, tune the lstm layers
                # l.trainable = False
    # sgd = op.SGD(lr=0.1, momentum=0.9, decay=1e-6)
    model.compile(loss='categorical_crossentropy', optimizer = 'adam',metrics=['accuracy'])
    model.summary()
    plot_model(model,to_file='model_scale'+scale+'_uwa'+'.png')
...
def main(epochsFt, lrFt, batch_sizeFt,decayFt):
    ...
    # model_scale1,test_score[0] = train_lstms(layers_num1,batch_size,joint_num1,Regs,filepath,stride_set,spp_numbers1,spp_numbers1_a,epoch=10,freeze = False,transfer = True,scale='1')
    # model_scale1.save('uwamodel_scale1_v2_ta.h5')    
    #########################################################
    joint_num2  = 11
    spp_numbers2 = [1,2,4]
    ...
    # model_scale2,test_score[1]=train_lstms(layers_num2,batch_size,joint_num2,Regs,filepath,stride_set,spp_numbers2,spp_numbers2_a,epoch=10,freeze = False,transfer = True,scale='2')
    # model_scale2.save('uwamodel_scale2_v2_ta.h5')
    ##########################################################
    joint_num3  = 15
    spp_numbers3 = [1,2,4]
    ...
    # model_scale3,test_score[2]=train_lstms(layers_num3,batch_size,joint_num3,Regs,filepath,stride_set,spp_numbers3,spp_numbers3_a,epoch=10,freeze = False,transfer = True, scale='3')
    # model_scale3.save('uwamodel_scale3_v2_ta.h5')
    ###########################################################
    ...
    model_scale1.load_weights('uwamodel_scale1_v2_ta.h5')
    model_scale2.load_weights('uwamodel_scale2_v2_ta.h5')
    model_scale3.load_weights('uwamodel_scale3_v2_ta.h5')
    ...
    print('Training duration (s) : ', time.time() - global_start_time)
    model.save('full_fusion_model_ta_uwa.h5')
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig("uwa_training_v4_sgd"+str(lrFt)+"_"+str(batch_sizeFt)+"_"+str(epochsFt)+".png")
    
    file = open('records.txt',"a+") 
    file.write('\n')
    file.write('uwa_training_v4_sgd'+str(lrFt)+'_'+str(batch_sizeFt)+'_'+str(epochsFt))
    file.write('\n')
