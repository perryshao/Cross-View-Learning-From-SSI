...
def calculate_ssm(filepath, scale):
    ...
    for sample in range(x_train.shape[0]):
        ...
        for t in range(x_train.shape[1]):
            ...
            # normalize the images with zero mean and unit variation
            SSM = (SSM - np.mean(SSM)) / (np.std(SSM) + 1e-8)
            x_train_ssm[sample_n, t, :, :, 0] = SSM
        x_train_ssm[sample_n, :, :, :, 0] = (
            x_train_ssm[sample_n, :, :, :, 0] - np.min(x_train_ssm[sample_n, :, :, :, 0])
        ) / (
            np.max(x_train_ssm[sample_n, :, :, :, 0])
            - np.min(x_train_ssm[sample_n, :, :, :, 0])
            + 1e-8
        )
        sample_n += 1
    ...
    for sample in range(x_test.shape[0]):
        ...
        x_test_ssm[sample_n, :, :, :, 0] = (
            x_test_ssm[sample_n, :, :, :, 0] - np.min(x_test_ssm[sample_n, :, :, :, 0])
        ) / (
            np.max(x_test_ssm[sample_n, :, :, :, 0])
            - np.min(x_test_ssm[sample_n, :, :, :, 0])
            + 1e-8
        )
        sample_n += 1
