"""Read HRI-UESTC skeletons for the arbitrary-view experimental subset.

Train on d != 8; test on the final tenth of each d8 sequence.
"""
# python -m pdb read_skeletons_uestc_av.py -p mat_from_skeleton/
import argparse
import os
...
datapath = '/home/data/HRI-UESTC/AV/'
ap = argparse.ArgumentParser()
ap.add_argument("-p", "--path", required=True, help="path to the skeleton mat data")
...
def build_x_repeat(x):
    ...
    x_repeat1 = np.zeros(
        [x.shape[0], x.shape[1], np.square((x.shape[-1] // 3)), 3], dtype="float32"
    )
...
for seq in seqs:
    ...
    if seq_name_list[1][1:] not in ['8']:
        ...
        if np.size(ske_xyz) > 0:
            ske_xyz = ske_xyz.reshape(-1, ske_xyz.shape[1] // 3, 3)  # (t,joint_num,3)
            segments = np.array_split(ske_xyz, 10, axis=0)
            # The held-out tenth can be empty for sequences shorter than ten frames.
            if segments[9].shape[0] == 0:
                continue
            test_data_cv.append(segments[9])
            test_labels.append(int(seq_name_list[0][1:]))
            frame_length.append(segments[9].shape[0])
...
# training set
x_train = np.zeros(
    [x_train_full.shape[0], x_train_full.shape[1], len(scale_list1), 3], dtype='float32'
)
...
x_train = np.zeros(
    [x_train_full.shape[0], x_train_full.shape[1], len(scale_list2), 3], dtype='float32'
)
for n in range(len(x_train_full)):
...
x_train = np.zeros(
    [x_train_full.shape[0], x_train_full.shape[1], len(scale_list3), 3], dtype='float32'
)
for n in range(len(x_train_full)):
...
# testing set
x_test = np.zeros(
    [x_test_full.shape[0], x_test_full.shape[1], len(scale_list1), 3], dtype='float32'
)
...
x_test = np.zeros(
    [x_test_full.shape[0], x_test_full.shape[1], len(scale_list2), 3], dtype='float32'
)
for n in range(len(x_test_full)):
    for s in range(len(scale_list2)):
...
x_test = np.zeros(
    [x_test_full.shape[0], x_test_full.shape[1], len(scale_list3), 3], dtype='float32'
)
for n in range(len(x_test_full)):
    for s in range(len(scale_list3)):
