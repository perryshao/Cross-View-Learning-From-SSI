function read_skeletons_mat(Dataset_Folder)
    ...
    for n = 1:sequences
        ...
        if ismember(str2num(file_name(6:8)), test_camera) % for testing
            for i = 1:framecount
                for j = 1:JOINT_NUM
                    ...
                    for k = 1:body_num
                        test_data_cv{count1, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                            bodyinfo(i).bodies(k).joints(j).x;
                        test_data_cv{count1, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                            bodyinfo(i).bodies(k).joints(j).y;
                        test_data_cv{count1, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                            bodyinfo(i).bodies(k).joints(j).z;
                    end
                    % When we observe just one skeleton, the second set is filled with zeros.
                    if body_num == 1
                        k = 2;
            ...
            % redefine the body_num, because body_num is various across time steps.
            body_num = size(test_data_cv{count1, 1}, 2) / 75;
            various = zeros(body_num, 1);
            % When we observed two or more, we decided about which one to be the main subject and
            % which one to be the second one, by measuring the amount of motion of their joints.
            if body_num > 1
                for bi = 1:body_num
                    various(bi) = sum(var(test_data_cv{count1, 1}(:, (bi - 1) * JOINT_NUM * 3 + ...
                        1:bi * JOINT_NUM * 3)));
                end
                [~, indx] = sort(various, 'descend');
                temp = zeros(framecount, JOINT_NUM * 3 * 2);
                temp(:, 1:JOINT_NUM * 3) = test_data_cv{count1, 1}(:, (indx(1) - 1) * JOINT_NUM ...
                    * 3 + 1:indx(1) * JOINT_NUM * 3);
                temp(:, JOINT_NUM * 3 + 1:end) = test_data_cv{count1, 1}(:, (indx(2) - 1) * ...
                    JOINT_NUM * 3 + 1:indx(2) * JOINT_NUM * 3);
                test_data_cv{count1, 1}(:, 1:JOINT_NUM * 3) = temp(:, 1:JOINT_NUM * 3);
                test_data_cv{count1, 1}(:, JOINT_NUM * 3 + 1:2 * JOINT_NUM * 3) = temp(:, ...
                    JOINT_NUM * 3 + 1:end);
        ...
        elseif ismember(str2num(file_name(6:8)), train_camera) % for training
            for i = 1:framecount
                for j = 1:JOINT_NUM
                    body_num = size(bodyinfo(i).bodies, 2);
                    for k = 1:body_num
                        train_data_cv{count2, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                            bodyinfo(i).bodies(k).joints(j).x;
                        train_data_cv{count2, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                            bodyinfo(i).bodies(k).joints(j).y;
                        train_data_cv{count2, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                            bodyinfo(i).bodies(k).joints(j).z;
                    end
                    % When we observe just one skeleton, the second set is filled with zeros.
                    if body_num == 1
            ...
            % redefine the body_num, because body_num is various across time steps.
            body_num = size(train_data_cv{count2, 1}, 2) / 75;
            various = zeros(body_num, 1);
            % When we observed two or more, we decided about which one to be the main subject and
            % which one to be the second one, by measuring the amount of motion of their joints.
            if body_num > 1
                for bi = 1:body_num
                    various(bi) = sum(var(train_data_cv{count2, 1}(:, (bi - 1) * JOINT_NUM * 3 + ...
                        1:bi * JOINT_NUM * 3)));
                end
                [~, indx] = sort(various, 'descend');
                temp = zeros(framecount, JOINT_NUM * 3 * 2);
                temp(:, 1:JOINT_NUM * 3) = train_data_cv{count2, 1}(:, (indx(1) - 1) * JOINT_NUM ...
                    * 3 + 1:indx(1) * JOINT_NUM * 3);
                temp(:, JOINT_NUM * 3 + 1:end) = train_data_cv{count2, 1}(:, (indx(2) - 1) * ...
                    JOINT_NUM * 3 + 1:indx(2) * JOINT_NUM * 3);
                train_data_cv{count2, 1}(:, 1:JOINT_NUM * 3) = temp(:, 1:JOINT_NUM * 3);
                train_data_cv{count2, 1}(:, JOINT_NUM * 3 + 1:2 * JOINT_NUM * 3) = temp(:, ...
                    JOINT_NUM * 3 + 1:end);
        ...
        %% Second, we collect data by the cross-subject protocol
        if ismember(str2num(file_name(10:12)), test_subjects) % for testing
            for i = 1:framecount
                for j = 1:JOINT_NUM
                    body_num = size(bodyinfo(i).bodies, 2);
                    for k = 1:body_num
                        test_data_cs{count3, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                            bodyinfo(i).bodies(k).joints(j).x;
                        test_data_cs{count3, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                            bodyinfo(i).bodies(k).joints(j).y;
                        test_data_cs{count3, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                            bodyinfo(i).bodies(k).joints(j).z;
                    end
                    % When we observe just one skeleton, the second set is filled with zeros.
                    if body_num == 1
            ...
            % redefine the body_num, because body_num is various across time steps.
            body_num = size(test_data_cs{count3, 1}, 2) / 75;
            various = zeros(body_num, 1);
            % When we observed two or more, we decided about which one to be the main subject and
            % which one to be the second one, by measuring the amount of motion of their joints.
            if body_num > 1
                for bi = 1:body_num
                    various(bi) = sum(var(test_data_cs{count3, 1}(:, (bi - 1) * JOINT_NUM * 3 + ...
                        1:bi * JOINT_NUM * 3)));
                end
                [~, indx] = sort(various, 'descend');
                temp = zeros(framecount, JOINT_NUM * 3 * 2);
                temp(:, 1:JOINT_NUM * 3) = test_data_cs{count3, 1}(:, (indx(1) - 1) * JOINT_NUM ...
                    * 3 + 1:indx(1) * JOINT_NUM * 3);
                temp(:, JOINT_NUM * 3 + 1:end) = test_data_cs{count3, 1}(:, (indx(2) - 1) * ...
                    JOINT_NUM * 3 + 1:indx(2) * JOINT_NUM * 3);
                test_data_cs{count3, 1}(:, 1:JOINT_NUM * 3) = temp(:, 1:JOINT_NUM * 3);
                test_data_cs{count3, 1}(:, JOINT_NUM * 3 + 1:2 * JOINT_NUM * 3) = temp(:, ...
                    JOINT_NUM * 3 + 1:end);
        ...
        elseif ismember(str2num(file_name(10:12)), train_subjects) % for training
            for i = 1:framecount
                for j = 1:JOINT_NUM
                    body_num = size(bodyinfo(i).bodies, 2);
                    for k = 1:body_num
                        train_data_cs{count4, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 1) = ...
                            bodyinfo(i).bodies(k).joints(j).x;
                        train_data_cs{count4, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 2) = ...
                            bodyinfo(i).bodies(k).joints(j).y;
                        train_data_cs{count4, 1}(i, (k - 1) * JOINT_NUM * 3 + (j - 1) * 3 + 3) = ...
                            bodyinfo(i).bodies(k).joints(j).z;
                    end
                    % When we observe just one skeleton, the second set is filled with zeros.
                    if body_num == 1
            ...
            % redefine the body_num, because body_num is various across time steps.
            body_num = size(train_data_cs{count4, 1}, 2) / 75;
            various = zeros(body_num, 1);
            % When we observed two or more, we decided about which one to be the main subject and
            % which one to be the second one, by measuring the amount of motion of their joints.
            if body_num > 1
                for bi = 1:body_num
                    various(bi) = sum(var(train_data_cs{count4, 1}(:, (bi - 1) * JOINT_NUM * 3 + ...
                        1:bi * JOINT_NUM * 3)));
                end
                [~, indx] = sort(various, 'descend');
                temp = zeros(framecount, JOINT_NUM * 3 * 2);
                temp(:, 1:JOINT_NUM * 3) = train_data_cs{count4, 1}(:, (indx(1) - 1) * JOINT_NUM ...
                    * 3 + 1:indx(1) * JOINT_NUM * 3);
                temp(:, JOINT_NUM * 3 + 1:end) = train_data_cs{count4, 1}(:, (indx(2) - 1) * ...
                    JOINT_NUM * 3 + 1:indx(2) * JOINT_NUM * 3);
                train_data_cs{count4, 1}(:, 1:JOINT_NUM * 3) = temp(:, 1:JOINT_NUM * 3);
                train_data_cs{count4, 1}(:, JOINT_NUM * 3 + 1:2 * JOINT_NUM * 3) = temp(:, ...
                    JOINT_NUM * 3 + 1:end);
