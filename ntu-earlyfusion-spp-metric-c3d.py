...
from keras.layers import BatchNormalization, Dense, Dropout, Flatten, Input, Lambda, LSTM
...
from ssi_layers import (
    flattenConv,
    flattenConv_output_shape,
    flattenSSM3,
    flattenSSM3_output_shape,
    repeat_x_groupjoint,
    repeat_x_groupjoint_output_shape,
    repeat_x_onejoint,
    repeat_x_onejoint_output_shape,
)
...
def get_model(summary=False):
    """Build the full Sports-1M C3D classifier."""
    model = Sequential()
    # 1st layer group
    model.add(
        Convolution3D(
    ...
    model.add(
        Convolution3D(
            128, [3, 3, 3], activation='relu', padding='same', name='conv2', strides=(1, 1, 1)
        )
    )
    ...
    model.add(
        Convolution3D(
            256, [3, 3, 3], activation='relu', padding='same', name='conv3a', strides=(1, 1, 1)
        )
    )
    model.add(
        Convolution3D(
            256, [3, 3, 3], activation='relu', padding='same', name='conv3b', strides=(1, 1, 1)
        )
    )
    ...
    model.add(
        Convolution3D(
            512, [3, 3, 3], activation='relu', padding='same', name='conv4a', strides=(1, 1, 1)
        )
    )
    model.add(
        Convolution3D(
            512, [3, 3, 3], activation='relu', padding='same', name='conv4b', strides=(1, 1, 1)
        )
    )
    ...
    model.add(
        Convolution3D(
            512, [3, 3, 3], activation='relu', padding='same', name='conv5a', strides=(1, 1, 1)
        )
    )
    model.add(
        Convolution3D(
            512, [3, 3, 3], activation='relu', padding='same', name='conv5b', strides=(1, 1, 1)
        )
    )
...
def get_int_model(layer, backend='tf', InputShape=None):
    ...
    int_model.add(
        Convolution3D(
            64, [3, 3, 3], activation='relu', padding='same', name='conv1', input_shape=input_shape
        )
    )
    if layer == 'conv1':
        return int_model
    int_model.add(
        MaxPooling3D(pool_size=(1, 2, 2), strides=(1, 2, 2), padding='valid', name='pool1')
    )
    if layer == 'pool1':
        return int_model

    # 2nd layer group
    int_model.add(Convolution3D(128, [3, 3, 3], activation='relu', padding='same', name='conv2'))
    if layer == 'conv2':
        return int_model
    int_model.add(
        MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2), padding='valid', name='pool2')
    )
    if layer == 'pool2':
        return int_model

    # 3rd layer group
    int_model.add(Convolution3D(256, [3, 3, 3], activation='relu', padding='same', name='conv3a'))
    if layer == 'conv3a':
        return int_model
    int_model.add(Convolution3D(256, [3, 3, 3], activation='relu', padding='same', name='conv3b'))
    if layer == 'conv3b':
        return int_model
    int_model.add(
        MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2), padding='valid', name='pool3')
    )
    if layer == 'pool3':
        return int_model

    # 4th layer group
    int_model.add(Convolution3D(512, [3, 3, 3], activation='relu', padding='same', name='conv4a'))
    if layer == 'conv4a':
        return int_model
    int_model.add(Convolution3D(512, [3, 3, 3], activation='relu', padding='same', name='conv4b'))
    if layer == 'conv4b':
        return int_model
    int_model.add(
        MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2), padding='valid', name='pool4')
    )
    if layer == 'pool4':
        return int_model

    # 5th layer group
    int_model.add(Convolution3D(512, [3, 3, 3], activation='relu', padding='same', name='conv5a'))
    if layer == 'conv5a':
        return int_model
    int_model.add(Convolution3D(512, [3, 3, 3], activation='relu', padding='same', name='conv5b'))
    if layer == 'conv5b':
        return int_model
    int_model.add(ZeroPadding3D(padding=(0, 1, 1), name='zeropad'))
    int_model.add(
        MaxPooling3D(pool_size=(2, 2, 2), strides=(2, 2, 2), padding='valid', name='pool5')
    )
    if layer == 'pool5':
        return int_model
...
if __name__ == '__main__':
    ...
    layers_num1 = [joint_num1, 4, 8, 16, 50, 2 * 2 * 16]
    joint_num2 = 12 * 2
    spp_numbers2 = [0, 1, 2]
    layers_num2 = [joint_num2, 8, 16, 32, 50, 2 * 2 * 32]
    joint_num3 = 25 * 2
    spp_numbers3 = [1, 2, 3]
    layers_num3 = [joint_num3, 16, 32, 64, 50, 4 * 4 * 64]
    lstm_layer_num = 100
    fc1_layer = 50
    print("[Info] Loading the Convolutional Layers for the First Stream")
    model_scale1 = get_int_model(
        'pool3', backend='tf', InputShape=(max_len, joint_num1, joint_num1, 3)
    )
    ...
    model_scale2 = get_int_model(
        'conv4b', backend='tf', InputShape=(max_len, joint_num2, joint_num2, 3)
    )
    model_scale2.summary()
    print("[Info] Loading the Convolutional Layers -- DONE!")
    print("[Info] Loading the Convolutional Layers for the Third Stream")
    model_scale3 = get_int_model(
        'conv4b', backend='tf', InputShape=(max_len, joint_num3, joint_num3, 3)
    )
    ...
    ssm_input1 = MetricLayer_ForC3D(joint_num1, kernel_regularizer=reg, name='metric_layer-1')(
        ssm_input1
    )
    ssm_input1 = Lambda(flattenSSM3, flattenSSM3_output_shape)(ssm_input1)
    ssm_input2 = MetricLayer_ForC3D(joint_num2, kernel_regularizer=reg, name='metric_layer-2')(
        ssm_input2
    )
    ssm_input2 = Lambda(flattenSSM3, flattenSSM3_output_shape)(ssm_input2)
    ssm_input3 = MetricLayer_ForC3D(joint_num3, kernel_regularizer=reg, name='metric_layer-3')(
        ssm_input3
    )
    ssm_input3 = Lambda(flattenSSM3, flattenSSM3_output_shape)(ssm_input3)

    # Load all three skeleton scales.
    ...
    z_out_f = Lambda(
        lambda x: K.sum(x, axis=1), output_shape=lambda s: (s[0], s[2]), name='Lambda2'
    )(lstm_output2)
    fc_softmax = Dense(
        class_num, activation='softmax', kernel_regularizer=reg, bias_regularizer=reg, name='FC2'
    )
    ...
    mgpu_model.summary()
    history1 = mgpu_model.fit(
        [x_train1, x_train2, x_train3], y_train1, batch_size=batch_size, epochs=epochs[0]
    )
    ...
    checkpoint = ModelCheckpoint(
        '/home/data/logs/' + model_filepath, monitor='val_acc', save_weights_only=True
    )
    history2 = mgpu_model.fit(
