...
# python -m pdb read_skeletons_ucla_cv.py --view_1 multiview_action/view_1 --view_2 multiview_action/view_2 --view_3 multiview_action/view_3
import numpy as np
import argparse
import os
import h5py
        ...
        try:
            with open(seq_path+'/'+files[files.index('fileList.txt')], 'r') as file:
                for line in file:
                    ...
                    ske_slides.append('frame_'+ske_slide[0]+'_tc_'+ske_slide[-1]+'_skeletons.txt')
        except Exception:
            print('no fileList.txt exist')
            continue
##         ske_seq = range(len(ske_slides))
        for ske_slide in ske_slides: 
            try:
                ...
                continue
        if ske_xyz != []:
            # if np.any(np.isnan(np.array(ske_xyz))):
                # np.array(ske_xyz)
...
## same with keras.utils.to_categorical funtion
lb = preprocessing.LabelBinarizer()
lb.fit(np.unique(y_label))
y_train = lb.transform(y_label)
## y_train = keras.utils.to_categorical(y_label, num_classes=10)
...
## y_test = keras.utils.to_categorical(y_label, num_classes=10)
...
x_train = np.zeros([x_train_full.shape[0],x_train_full.shape[1],len(scale1_list2),3], dtype='float32')
for n in range(len(x_train_full)):
...
for n in range(len(x_train_full)):
    ...
    x_train[n][:,s+1:s+padding_num+1] = np.zeros([padding_num,3],dtype='float32') ## padding the joints which are 21-25, with zeros.
...
x_test = np.zeros([x_test_full.shape[0],x_test_full.shape[1],len(scale1_list2),3], dtype='float32')
for n in range(len(x_test_full)):
    for s in range(len(scale1_list2)):
...
for n in range(len(x_test_full)):
    ...
    x_test[n][:,s+1:s+padding_num+1] = np.zeros([padding_num,3],dtype='float32') ## padding the joints which are 21-25, with zeros.
