...
# python -m pdb read_skeletons_uwa_cv.py --view_1 ActionsSkeleton/view_1/ --view_2
# ActionsSkeleton/view_2/ --view_3 ActionsSkeleton/view_3/ --view_4 ActionsSkeleton/view_4/
import numpy as np
import argparse
import os
import h5py
...
ap.add_argument("-V4", "--view_4", required=True, help="path to the view 4 skeletons")
...
padding_num = 1
x_train = np.zeros(
    [x_train_full.shape[0], x_train_full.shape[1], len(scale1_list2) + padding_num, 3],
    dtype='float32',
)
for n in range(len(x_train_full)):
    for s in range(len(scale1_list2)):
        x_train[n][:, s] = x_train_full[n, :, scale1_list2[s] - 1]
    # padding the joints which are 12, with zeros.
...
# to pad the zeros to the joints that are beyond the 15th joint. so we do +10 on len(scale1_list3)
padding_num = 10
x_train = np.zeros(
...
for n in range(len(x_train_full)):
    ...
    # padding the joints which are 16-25, with zeros.
    x_train[n][:, s + 1 : s + padding_num + 1] = np.zeros([padding_num, 3], dtype='float32')
...
padding_num = 1
x_test = np.zeros(
    [x_test_full.shape[0], x_test_full.shape[1], len(scale1_list2) + padding_num, 3],
    dtype='float32',
)
for n in range(len(x_test_full)):
    for s in range(len(scale1_list2)):
